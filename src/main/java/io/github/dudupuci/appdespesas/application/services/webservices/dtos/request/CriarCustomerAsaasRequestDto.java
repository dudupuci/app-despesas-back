...
import io.github.dudupuci.appdespesas.domain.exceptions.CampoObrigatorioException;
import io.github.dudupuci.appdespesas.domain.entities.UsuarioSistema;
import io.github.dudupuci.appdespesas.domain.utils.AppDespesasUtils;
import org.apache.commons.lang3.StringUtils;

import static io.github.dudupuci.appdespesas.domain.utils.AppDespesasUtils.validaString;
...
public record CriarCustomerAsaasRequestDto(
    ...
    public static CriarCustomerAsaasRequestDto fromUsuarioSistema(UsuarioSistema usuarioSistema) {
        ...
        if (StringUtils.isEmpty(usuarioSistema.getNomeCompleto())) {
            throw new CampoObrigatorioException("Nome completo não informado no perfil do usuário. Será necessário preencher esse campo para dar continuidade ao processo.");
        }

        if (StringUtils.isEmpty(usuarioSistema.getCpfCnpj())) {
            throw new CampoObrigatorioException("CPF/CNPJ não informados no perfil do usuário. Será necessário preencher esse campo para dar continuidade ao processo.");
        }
