...
@Repository
public interface MovimentacoesRepository extends JpaRepository<JpaMovimentacao, Long> {

    /**
     * Lista todas as movimentações de um usuário com filtros opcionais
     */
    @Query("SELECT m FROM JpaMovimentacao m WHERE m.usuarioSistema.id = :usuarioId " +
           "ORDER BY m.dataDaMovimentacao DESC")
    List<JpaMovimentacao> listarTodasPorUsuarioId(@Param("usuarioId") UUID usuarioId);
    ...
    @Query("SELECT m FROM JpaMovimentacao m WHERE m.usuarioSistema.id = :usuarioId " +
           ...
           "ORDER BY m.dataDaMovimentacao DESC")
    List<JpaMovimentacao> listarPorUsuarioIdETipo(
    ...
    @Query("SELECT m FROM JpaMovimentacao m WHERE m.usuarioSistema.id = :usuarioId " +
           ...
           "ORDER BY m.dataDaMovimentacao DESC")
    List<JpaMovimentacao> listarPorUsuarioIdEPeriodo(
            @Param("usuarioId") UUID usuarioId,
    ...
    @Query("SELECT m FROM JpaMovimentacao m WHERE m.usuarioSistema.id = :usuarioId " +
           ...
           "AND m.dataDaMovimentacao BETWEEN :dataInicio AND :dataFim " +
           "ORDER BY m.dataDaMovimentacao DESC")
    List<JpaMovimentacao> listarPorUsuarioIdTipoEPeriodo(
            @Param("usuarioId") UUID usuarioId,
            @Param("tipoMovimentacao") TipoMovimentacao tipoMovimentacao,
            @Param("dataInicio") Date dataInicio,
            @Param("dataFim") Date dataFim
    ...
    List<JpaMovimentacao> buscarPorData(@Param("usuarioId") UUID usuarioId, @Param("data") Date data);
}
